  ...
  <v-app id="app">
    ...
    >
      <v-btn
        ...
        >Dashboard</v-btn
      >
      <v-btn @click="$router.push({path: '/'})" v-else>Back to Site</v-btn>
      <v-btn v-if="$route.query.edit" @click="save_changes">Save Changes</v-btn>
    ...
    <v-card
      ...
      v-if="user && route_paths.includes($route.path) && $route.query.edit"
    >
      <div class="mt-5">
        <center>
          <div v-for="element in elements" :key="element.name">
            <v-btn @click="add_element(element)">{{ element.name }}</v-btn>
            <br />
          </div>
        </center>
      </div>
...
export default {
      ...
      if (
        !local_page.data.every((data, idx) =>
          Object.values(data).every(
            (value, idx2) =>
              page.data[idx] && value == Object.values(page.data[idx])[idx2],
          ),
        ) ||
  ...
  created() {
    docData(db.collection('settings').doc('defaults')).subscribe(defaults => {
      if (defaults.menu) {
        docData(db.collection('menus').doc(defaults.menu)).subscribe(menu => {
